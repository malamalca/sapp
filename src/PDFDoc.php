...
class PDFDoc extends Buffer {
    ...
    protected function _generate_signature_in_document() {
        ...
        if ($this->_certificate !== null) {
            ...
            // Create a placeholder signature object (for deferred/client-side signing)
            $signature = $this->create_object([], PDFSignatureObject::class, false);
     ...
     * @param rebuild - if true, the document will be rebuilt to remove the unused objects and the old versions of them (if any)
     * @return array [offsets, end_offset] offsets indexed by oid, and end_offset is the total byte offset at the end
     */
    ...
    /**
     * Generates the trailer for the document (for use with to_pdf_with_signature_placeholder).
    ...
    /**
     * Obtains the target version for the PDF document
     * @return string the version string for the PDF document (e.g. 1.5)
     */
    ...
    /**
     * Outputs the document to a buffer with a signature placeholder (for client-side signing).
     * The placeholder consists of <00000...0000> where the amount of 0s is defined by __SIGNATURE_MAX_LENGTH.
     * @param rebuild whether we are rebuilding the whole xref table or not
    ...
    public function to_pdf_with_signature_placeholder($rebuild = false) : Buffer {
        ...
        // Calculate trailer first (may add objects)
        ...
        // Calculate sizes for ByteRange: prev = offset of sig object, post = trailer size
        [ $offsets, $eod_offset ] = $this->_get_object_offsets($rebuild);
        $_signature->set_sizes($offsets[$_signature->get_oid()], $pdf_trailer->size());

        // Regenerate content with correct ByteRange
        $pdf_content = $this->_generate_PDF_content($rebuild);
